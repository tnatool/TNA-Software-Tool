        	...
        	return { 
        		"show" : {
                    ...
                    "action" : function (node) { 
                    	...
                    	case "agency":                    		
                    		if ($.jstree._reference("#list")._is_loaded(node)){
                    			$.each($.jstree._reference("#list")._get_children(node), function(i,child){
                    				if ($.jstree._reference("#list")._is_loaded(child)){
                    					$.each($.jstree._reference("#list")._get_children(child), function(i,gchild){
            								$.jstree._reference("#list").change_state(gchild, false);
            								});
                    				}else{
                    					$.jstree._reference("#list").load_node_json(child, function(){$.each($.jstree._reference("#list")._get_children(child), function(i,gchild){
            								$.jstree._reference("#list").change_state(gchild, false);
            								});},function(){alert("Node Load Error");});
                    				}                        			
                                		});
                			} else{
                				$.jstree._reference("#list").load_node_json(node,function(){$.each($.jstree._reference("#list")._get_children(node), function(i,child){
                					if ($.jstree._reference("#list")._is_loaded(child)){
                						$.each($.jstree._reference("#list")._get_children(child), function(i,gchild){
            								$.jstree._reference("#list").change_state(gchild, false);
            								});
                    	...
                    	case "route":
                    		if ($.jstree._reference("#list")._is_loaded(node)){
	                    		$.each($.jstree._reference("#list")._get_children(node), function(i,child){
	                    		$.jstree._reference("#list").change_state(child, false);
	                    		}); 
                    		}else {
                    			$.jstree._reference("#list").load_node_json(node, function(){$.each($.jstree._reference("#list")._get_children(node), function(i,child){
                            		$.jstree._reference("#list").change_state(child, false);
                ...
                "hide" : {
                    ...
                    "action" : function (node) { 
                    	...
                    	case "agency":                    		
                    		$.each($.jstree._reference("#list")._get_children(node), function(i,child){
                    			$.each($.jstree._reference("#list")._get_children(child), function(i,gchild){
                            		$.jstree._reference("#list").uncheck_node(gchild);
                            		});
                        		});
	                    	break; 
                    	case "route":
                    		$.each($.jstree._reference("#list")._get_children(node), function(i,child){
                    		$.jstree._reference("#list").uncheck_node(child);
                    		});
                    		break;
                    	}
                    }
...
.bind("change_state.jstree", function (e, d) {
    ...
    if ((tagName == "A" || tagName == "INS"|| tagName == "LI")&&(refreshing != true && refreshing != "undefined")) {
    	...
    	case "agency":    		
    		...
    		//alert(mynode.length);
    		//checkbox is checked
    		if ($.jstree._reference("#list").is_checked(node)){    			
    			//if it has any checked children   			
    			$.jstree._reference("#list")._get_children(node).each( function( idx, listItem ) {    				
    				if ($.jstree._reference("#list").is_checked($(listItem))){
	    				...
	    				$.jstree._reference("#list").uncheck_node($(listItem));
    				};
                 });
    			...
    			getdata(1,node.attr("id"),"","",Layers%6,disponmap,$.jstree._reference("#list").get_text(node));
    			Layers = Layers + 1;
    			...
    			if ((($("#list").jstree("get_checked",node,true)).length)>0) node.css("opacity", "0.6");
    			stops.eachLayer(function (layer) {
    	...
    	case "route":
    		...
    		if ($.jstree._reference("#list").is_checked(node)){
    			...
    			rparent = $.jstree._reference("#list")._get_parent(node);
    			//parent = d.inst._get_parent(node);
    			if($.jstree._reference("#list").is_checked(rparent)){
    			//if((parent.length)>0){
    				//Layers = Layers -1;
    				stops.eachLayer(function (layer) {
    				...
    				$.jstree._reference("#list").uncheck_node(rparent);
    			}
    			//alert(d.rslt.attr("type")+" : "+d.inst._get_parent(d.rslt).attr("id")+" : "+d.rslt.attr("id"));
    			//var nodeid = d.rslt.attr("id").split("_")[1];
    			//alert(nodeid);
    			//$.jstree._reference("#list").uncheck_node(d.inst._get_parent(d.rslt));
    			...
    			getdata(2,rparent.attr("id"),node.attr("id"),"",Layers%6,disponmap,$.jstree._reference("#list").get_text(node));
    			Layers = Layers + 1;
    			//var marker = L.marker([44.574606,-123.27987]).addTo(stops);
    		} else {    			
    			//mynode = $("#" + node.attr("id") + ".jstree-unchecked");
    			...
    			if ((($("#list").jstree("get_checked",rparent,true)).length)==0) rparent.css("opacity", "1");
    			node.css("background-color","");
    		...
    		break;
    	case "variant":
    		vparent = $.jstree._reference("#list")._get_parent(node);
    		//alert($.jstree._reference("#list").is_checked(node));
    		//mynode = $("#" + node.attr("id")+ ".jstree-checked");    		
    		...
    		if ($.jstree._reference("#list").is_checked(node)){
    			...
    			$.jstree._reference("#list")._get_parent(vparent).css("opacity", "0.6");
    			...
    			//Layers = Layers -1;
    			node.css("background-color","");
    			if ((($("#list").jstree("get_checked",vparent,true)).length)==0) {
    				vparent.css("font-weight", "normal");
    				if ((($("#list").jstree("get_checked",$.jstree._reference("#list")._get_parent(vparent),true)).length)==0) {
    					$.jstree._reference("#list")._get_parent(vparent).css("opacity", "1");
    				}
    			}    			
